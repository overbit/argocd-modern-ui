(() => {
  ...
  function renderApps(filter = '') {
    ...
    shell(`<div class="hero"><div><h1>Applications</h1><p>Operational overview with an interactive topology for every application.</p></div><div class="hero-actions"><button class="button" data-parity-path="/applications" data-parity-title="Application controls" data-parity-description="Create, edit, delete, filter, and operate applications using Argo CD's complete native controls.">Complete controls</button></div></div><div class="stats"><div class="stat"><span>Total</span><b>${state.apps.length}</b></div><div class="stat"><span>Healthy</span><b>${healthy}</b></div><div class="stat"><span>Needs attention</span><b>${problem}</b></div><div class="stat"><span>Progressing</span><b>${progressing}</b></div></div><div class="panel"><div class="panel-head"><b>${visible.length} applications</b><input id="app-filter" value="${esc(filter)}" placeholder="Filter applications" aria-label="Filter applications"></div>${rows || '<div class="empty">No applications found.</div>'}</div>`, 'Applications');
  ...
  function renderApp() {
    ...
    if (state.appTab === 'controls') {
      const controls = native ? native.frame(appNativeRoute, {
        title: `${app?.metadata?.name || 'Application'} controls`,
        description: 'Full native operations, resource details, manifests, diffs, events, logs, history, rollback, and RBAC-aware dialogs.'
      }) : '<div class="panel error">Complete controls are unavailable.</div>';
      shell(`${hero}${summary}${tabs}${controls}`, app?.metadata?.name || 'Application');
      return;
    }
    ...
    if (!graph) {
      ...
      shell(`${hero}${summary}${tabs}<div class="detail"><div class="card"><h3>Resources · ${nodes.length}</h3>${resources || '<div class="empty">No resources returned.</div>'}</div><aside class="card"><h3>Application status</h3><div class="kv"><span>Health</span>${badge(health(app))}</div><div class="kv"><span>Sync</span>${badge(sync(app))}</div><div class="kv"><span>Destination</span><b>${esc(app?.spec?.destination?.namespace || '—')}</b></div><div class="kv"><span>Repository</span><b>${esc(repo)}</b></div></aside></div>`, app?.metadata?.name || 'Application');
      return;
    }

    shell(`${hero}${summary}${tabs}<section class="topology-panel">${graph.toolbar(nodes)}${graph.content(nodes)}</section>`, app?.metadata?.name || 'Application');
  }
  ...
  async function navigate(view, force = false) {
    ...
    try {
      if (view === 'applications') {
        ...
        await openApp(key(state.selected));
      }
    } catch (err) {
  ...
  async function syncSelected() {
    ...
    try {
      await api(`/api/v1/applications/${name}/sync`, {method:'POST', body:JSON.stringify({appNamespace: state.selected.metadata?.namespace || ''})});
      await openApp(key(state.selected));
    } catch (err) {
      error(`Sync failed: ${err.message || err}`, state.selected.metadata?.name || 'Application');
    }
  }
