(() => {
  ...
  function renderApp() {
    ...
    const nodes = [...(state.tree?.nodes || []), ...(state.tree?.orphanedNodes || [])].sort((a,b) => ['Degraded','Missing'].includes(a?.health?.status) ? -1 : ['Degraded','Missing'].includes(b?.health?.status) ? 1 : 0);
  ...
  async function openApp(appKey) {
    ...
    try { [state.selected, state.tree] = await Promise.all([api(`/api/v1/applications/${name}${query}`), api(`/api/v1/applications/${name}/resource-tree${query}`)]); renderApp(); }
